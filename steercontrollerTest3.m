...
save_msg = 0; test_msg = 'Control'; case_msg = 'Steer_ParaVariation2'; legcommand = 0;
err = 0;

for i = 1:length(des_var)
...
for i = 1:length(des_var)
    ...
    hold on
    leg(i) = cellstr(['\beta = ',num2str(des_var(i))]);
end
xlabel('time(s)')
ylabel('velocity(m/s)')
...
title('XY Plane')
grid on
xlim([0 max(abs(max([StateVec{1}(:,7), StateVec{2}(:,7), StateVec{3}(:,7)])))*1.25]);
hleg = legend(leg,'location','northwest');
if legcommand == 1
    htitle = get(hleg,'Title');
    set(htitle,'String','Commanded Heading (deg)')
...
figure(19)
for i = 1:length(des_var)
    plot(TVec{i},StateVec{i}(:,15),'LineWidth',2);
    hold on
    leg(i) = cellstr(['\zeta = ',num2str(des_var(i))]);
end
xlabel('time(s)')
...
figure(20)
for i = 1:length(des_var)
    plot(TVec{i},StateVec{i}(:,16),'LineWidth',2);
    hold on
    leg(i) = cellstr(['\zeta = ',num2str(des_var(i))]);
end
xlabel('time(s)')
