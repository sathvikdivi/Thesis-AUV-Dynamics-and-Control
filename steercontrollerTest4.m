...
while (told <= tend)
    ...
    told = t(end);
    init = [u,v,w,p,q,r,x,y,z,phi,theta,psi];
    StateVec = vertcat(StateVec,[init,delR,sigmaSteer,Xprops,sigmaSurge]);
...
figure(16)
plot(TVec,StateVec(:,15),'LineWidth',2);
xlabel('time(s)')
...
figure(17)
plot(TVec,StateVec(:,16),'LineWidth',2);
xlabel('time(s)')
ylabel('sigma')
grid on
